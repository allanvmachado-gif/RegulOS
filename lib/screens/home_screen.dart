// ═══════════════════════════════════════════════════════════
//  home_screen.dart — com botão de sync e logout
// ═══════════════════════════════════════════════════════════
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final List<Widget> _screens = const [
    RotinaScreen(), TarefasScreen(), ReunioesScreen(),
    CompromissosScreen(), DiarioScreen(), StatsScreen(), PerfilScreen(),
  ];
        ...
        return Stack(
          children: [
            Scaffold(
              ...
              appBar: _currentIndex == 0 ? null : null,
              body: IndexedStack(
                index: _currentIndex,
                children: _screens,
              ),
                ...
                children: [
                      ...
                      children: [
                        ...
                        if (state.currentUser != null) ...[
                          CircleAvatar(
                            ...
                            child: Text(
                              (state.currentUser!.displayName ?? 'A')[0].toUpperCase(),
                  ...
                  Container(
                    ...
                    child: BottomNavigationBar(
                      ...
                      items: const [
                        BottomNavigationBarItem(icon: Icon(Icons.view_day_rounded), label: 'Rotina'),
                        BottomNavigationBarItem(icon: Icon(Icons.task_alt_rounded), label: 'Tarefas'),
                        BottomNavigationBarItem(icon: Icon(Icons.groups_rounded), label: 'Reuniões'),
                        BottomNavigationBarItem(icon: Icon(Icons.event_rounded), label: 'Compromissos'),
                        BottomNavigationBarItem(icon: Icon(Icons.auto_stories_rounded), label: 'Diário'),
